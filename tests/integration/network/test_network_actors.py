...
def test_all_ursulas_know_about_all_other_ursulas(ursulas, test_registry):
    ...
    """
    onchain_records = [(u.staking_provider_address,) for u in ursulas]
    ...
    for record in onchain_records:
        ...
        for propagating_ursula in ursulas[:1]:  # Last Ursula is not staking
            if address == propagating_ursula.checksum_address:
                ...
                assert (
                    address in propagating_ursula.peers.addresses()
                ), "{} did not know about {}".format(
                    propagating_ursula, Nickname.from_seed(address)
                )
...
def test_vladimir_illegal_interface_key_does_not_propagate(ursulas):
    ...
    other_ursula.block_until_specific_nodes_are_known(
        [vladimir.staking_provider_address]
    )
    ...
    assert (
        "Peer " + str(vladimir_as_learned.checksum_address) + " is invalid" in warning
    )
...
def test_alice_refuses_to_select_node_unless_ursula_is_valid(
    ...
    vladimir = Vladimir.from_target_ursula(
        target, substitute_verifying_key=True, sign_metadata=True
    )
