...
def test_bridge_fulfiller_execute_intent_condition(
    ...
    intent_tx = orderbook_contract.submitIntent(
        order_id, receiver, value=target_amount, sender=fulfiller  # fulfill with ETH
    )
    intent_id = 0  # First intent gets ID 0

    # Create call data for executeIntent(intentId, receiver)
    call_data = encode_function_call(
        "executeIntent(uint256,address)", [intent_id, receiver.address]
    )
...
def test_bridge_fulfiller_claim_condition(
    ...
    # Execute the intent first (so it gets mapped to fulfilledOrders)
    # This is needed because getIntentID only returns non-zero after execution
    ...
    # Create call data for claimOrder(orderId, fulfillerAddress) 
    call_data = encode_function_call(
        "claimOrder(uint256,address)", [order_id, fulfiller.address]
    )
    signing_object = mocker.Mock()
    signing_object.call_data = bytes(call_data)
    context = {SIGNING_CONDITION_OBJECT_CONTEXT_VAR: signing_object}
    ...
    # Verify the condition with proper context (merge signing context with orderID)
    full_context = {**context, ":orderID": order_id}
    allowed, result = fulfiller_claim_condition.verify(providers=condition_providers, **full_context)
    assert allowed is True, f"Condition should be allowed, but got result: {result}"

    print("✅ Bridge fulfiller claim condition passed!")
    print(f"OrderBook contract address: {orderbook_contract.address}")
    print(f"Intent executed and order claimed successfully!")
...
def test_orderbook_complete_bridge_flow_with_tokens(
    ...
    order_status_condition = ContractCondition(
        ...
        parameters=[order_id],
        return_value_test=ReturnValueTest(
            comparator="==", value=OrderState.AWAITING_FULFILLMENT.value
        ),
    )
    ...
    intent_status_condition = ContractCondition(
        ...
        return_value_test=ReturnValueTest(
            comparator="==", value=IntentState.AWAITING_EXECUTION.value
        ),
    )
    ...
    allowed, result = order_id_from_intent_condition.verify(
    ...
    intent_details_condition = ContractCondition(
        ...
        parameters=[intent_id],
        return_value_test=ReturnValueTest(
            comparator="==", value=[ritual_token.address, target_amount]
        ),
    )
...
def test_bridge_state_transitions_with_execution(
    ...
    intent_tx = orderbook_contract.submitIntent(
        order_id, receiver, value=target_amount, sender=fulfiller
    )
    intent_id = 0

    # Execute intent (as threshold signer)
    execute_tx = orderbook_contract.executeIntent(
        intent_id, receiver, sender=deployer_account  # threshold signer
    )
    ...
    intent_status_condition = ContractCondition(
        ...
        parameters=[intent_id],
        return_value_test=ReturnValueTest(
            comparator="==", value=IntentState.EXECUTED.value
        ),
    )
    ...
    allowed, result = intent_id_from_order_condition.verify(
        providers=condition_providers
    )
