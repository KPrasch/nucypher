...
def test_nucypher_help_message(click_runner, command):
    ...
    assert result.exit_code == 0
    assert (
        "[OPTIONS] COMMAND [ARGS]" in result.output
    ), "Missing or invalid help text was produced."
    assert all(e in result.output for e in entry_points)


@pytest.mark.parametrize(
    "entry_point_name, entry_point",
    ([command.name, command] for command in ENTRY_POINTS),
)
def test_character_help_messages(click_runner, entry_point_name, entry_point):
    ...
    assert result.exit_code == 0
    assert (
        f"{entry_point_name}" in result.output
    ), "Missing or invalid help text was produced."
    if isinstance(entry_point, click.Group):
        for sub_command, config in entry_point.commands.items():
            if not config.hidden:
                assert (
                    f"{sub_command}" in result.output
                ), f"Sub command {sub_command} is missing from help text"
            else:
                assert (
                    f"{sub_command}" not in result.output
                ), f"Hidden command {sub_command} in help text"
...
def test_character_sub_command_help_messages(
    click_runner, entry_point_name, entry_point
):
    if isinstance(entry_point, click.Group):
        for sub_command in entry_point.commands:
            ...
            assert result.exit_code == 0
            assert (
                f"{entry_point_name} {sub_command}" in result.output
...
def test_echo_config_root(click_runner):
    ...
    assert result.exit_code == 0
    assert (
        str(DEFAULT_CONFIG_ROOT.absolute()) in result.output
    ), "Configuration path text was not produced."
...
def test_echo_logging_root(click_runner):
    ...
    assert result.exit_code == 0
    assert (
        str(USER_LOG_DIR.absolute()) in result.output
    ), "Log path text was not produced."
