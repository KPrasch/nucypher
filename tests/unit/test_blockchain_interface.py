...
def test_use_pending_nonce_when_building_payload(
    mock_testerchain, mocker, random_address
):
    ...
    mock_testerchain.client.w3.eth.get_transaction_count = mocker.Mock(
        side_effect=mock_get_transaction_count
    )
    ...
    payload = mock_testerchain.build_payload(
        sender_address=sender, payload=None, use_pending_nonce=True
    )
    assert payload["nonce"] == 0
    payload = mock_testerchain.build_payload(
        sender_address=sender, payload=None, use_pending_nonce=False
    )
    assert payload["nonce"] == 0
