...
def test_request_with_retry(retry_middleware_class):
    retries = 4
    make_request = Mock()
    retry_middleware = retry_middleware_class(
        make_request=make_request, w3=Mock(), retries=retries, exponential_backoff=False
    )

    # Retry Case - RPCResponse fails due to limits, and retry required
    make_request.return_value = RPC_TOO_MANY_REQUESTS

    retry_response = retry_middleware(
        method=RPCEndpoint("web3_client_version"), params=None
    )
...
def test_request_with_non_retry_exception(retry_middleware_class):
    ...
    retry_middleware = retry_middleware_class(
        make_request=make_request, w3=Mock(), exponential_backoff=False
    )
...
def test_request_success_with_no_retry(retry_middleware_class):
    ...
    retry_middleware = retry_middleware_class(
        make_request=make_request, w3=Mock(), retries=10, exponential_backoff=False
    )
    retry_response = retry_middleware(
        method=RPCEndpoint("web3_client_version"), params=None
    )
    ...
    assert (
        make_request.call_count == 1
    )  # first call was successful, no need for retries
...
def test_alchemy_request_with_retry():
    ...
    test_responses = [
        ...
        RPCResponse(
            error="Your app has exceeded its compute units per second capacity. If you have retries enabled, "
