...
def test_invalid_keystore_filepath_parts(tmp_path, tmp_path_factory):
    # Setup
    not_hex = "h" + "".join(
        random.choice(string.ascii_letters) for _ in range(Keystore._ID_SIZE)
    )
    invalid_paths = (
    ...
    for invalid_path in invalid_paths:
        ...
        with pytest.raises(
            Keystore.Invalid, match=f"{invalid_path} is not a valid keystore filename"
        ):
...
def test_invalid_keystore_file_type(tmp_path, tmp_path_factory):
    ...
    with pytest.raises(
        Keystore.NotFound, match=f"Keystore '{invalid_path.absolute()}' does not exist."
    ):
        _keystore = Keystore(invalid_path)
...
def test_keystore_instantiation_defaults(tmp_path_factory):
    ...
    keystore_id = "".join(
        random.choice(string.hexdigits.lower()) for _ in range(Keystore._ID_SIZE)
    )
    ...
    assert (
        parent in keystore.keystore_filepath.parents
...
def test_keystore_generation_defaults(tmp_path_factory, mnemonic):
    ...
    keystore = Keystore.from_mnemonic(
        mnemonic=mnemonic, password=INSECURE_DEVELOPMENT_PASSWORD, keystore_dir=parent
    )
    assert not keystore.is_unlocked  # defaults to locked
    assert keystore._Keystore__secret is KEYSTORE_LOCKED
    assert (
...
def test_keystore_invalid_password(tmpdir, mnemonic):
    with pytest.raises(InvalidPassword):
        _keystore = Keystore.from_mnemonic(
            password="short", keystore_dir=tmpdir, mnemonic=mnemonic
        )


def test_keystore_generate_report_interactive_false(tmpdir, mnemonic):
    _keystore = Keystore.from_mnemonic(
        password=INSECURE_DEVELOPMENT_PASSWORD, keystore_dir=tmpdir, mnemonic=mnemonic
    )
...
def test_keystore_derive_crypto_power_without_unlock(tmpdir, mnemonic):
    keystore = Keystore.from_mnemonic(
        mnemonic=mnemonic, password=INSECURE_DEVELOPMENT_PASSWORD, keystore_dir=tmpdir
    )
...
def test_keystore_serializer():
    encrypted_secret, psalt, wsalt = (
        b"peanuts! Get your peanuts!",
        b"sea salt",
        b"bath salt",
    )
    payload = _assemble_keystore(
        encrypted_secret=encrypted_secret, password_salt=psalt, wrapper_salt=wsalt
    )
...
def test_write_keystore_file(temp_dir_path):
    ...
    deserialized_payload_from_file = _read_keystore(
        path=temp_filepath, deserializer=_deserialize_keystore
    )
...
def test_restore_keystore_from_mnemonic(tmpdir, mocker):
    ...
    # Decrypt post-generation
    keystore = Keystore.from_mnemonic(
        mnemonic=words, password=INSECURE_DEVELOPMENT_PASSWORD, keystore_dir=tmpdir
    )
    keystore.unlock(password=INSECURE_DEVELOPMENT_PASSWORD)
...
def test_derive_hosting_power(tmpdir, mnemonic):
    ...
    keystore.unlock(password=INSECURE_DEVELOPMENT_PASSWORD)
    hosting_power = keystore.derive_crypto_power(
    ...
    rederived_hosting_power = keystore.derive_crypto_power(
        power_class=TLSHostingPower, host=LOOPBACK_ADDRESS
    )
    assert (
        hosting_power.public_key().public_numbers()
        == rederived_hosting_power.public_key().public_numbers()
    )
