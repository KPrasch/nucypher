...
upvest_json = {
    ...
    "estimates": {"fastest": 105.2745, "fast": 97.158, "medium": 91.424, "slow": 87.19},
}
...
def test_canonical_speed_names():
    ...
    for canonical_speed, equivalence_class in speed_equivalence_classes.items():
        for speed_name in equivalence_class:
            assert canonical_speed == EthereumGasPriceDatafeed.get_canonical_speed(
                speed_name
            )
    ...
    for wrong_name, suggestion in similarities:
        message = (
            f"'{wrong_name}' is not a valid speed name. Did you mean '{suggestion}'?"
        )
...
def test_etherchain():
    ...
    with patch(
        "nucypher.utilities.datafeeds.EtherchainGasPriceDatafeed._parse_gas_prices",
        autospec=True,
    ):
        EtherchainGasPriceDatafeed.gas_prices = dict()
        with patch.dict(
            EtherchainGasPriceDatafeed.gas_prices, values=parsed_gas_prices
        ):
...
def test_upvest():
    ...
    with patch(
        "nucypher.utilities.datafeeds.UpvestGasPriceDatafeed._parse_gas_prices",
        autospec=True,
    ):
...
def test_zoltu():
    ...
    with patch(
        "nucypher.utilities.datafeeds.ZoltuGasPriceDatafeed._parse_gas_prices",
        autospec=True,
    ):
...
def test_datafeed_median_gas_price_strategy():
    ...
    with patch(
        "nucypher.utilities.datafeeds.UpvestGasPriceDatafeed.construct_gas_strategy",
        ...
        with patch(
            "nucypher.utilities.datafeeds.ZoltuGasPriceDatafeed.construct_gas_strategy",
            ...
            assert datafeed_median_gas_price_strategy("web3", "tx") == median(
                [mock_upvest_gas_price, mock_zoltu_gas_price]
            )

    # If, for example, Upvest fails, the median is computed using the other two feeds
    with patch(
        "nucypher.utilities.datafeeds.UpvestGasPriceDatafeed._probe_feed",
    ...
    with patch(
        ...
        with patch(
            ...
            datafeed_median_gas_price_strategy = construct_datafeed_median_strategy()
            assert (
                datafeed_median_gas_price_strategy("web3", "tx") == mock_zoltu_gas_price
            )
    ...
    with patch(
        ...
        with patch(
            "nucypher.utilities.datafeeds.ZoltuGasPriceDatafeed._probe_feed",
            side_effect=Datafeed.DatafeedError,
        ):
            with patch(
                "nucypher.utilities.gas_strategies.rpc_gas_price_strategy",
                side_effect=construct_mock_gas_strategy(mock_rpc_gas_price),
            ):
                datafeed_median_gas_price_strategy = (
                    construct_datafeed_median_strategy()
                )
                assert (
                    datafeed_median_gas_price_strategy("web3", "tx")
                    == mock_rpc_gas_price
                )
