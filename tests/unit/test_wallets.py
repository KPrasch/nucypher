...
def test_invalid_keystore(tmp_path, capture_wallets):
    with pytest.raises(FileNotFoundError):
        LocalAccount.from_keystore(
            tmp_path.absolute() / "nonexistent", INSECURE_DEVELOPMENT_PASSWORD
        )
...
def test_signer_reads_keystore_from_disk(temp_dir_path, capture_wallets):
    ...
    try:
        ...
        wallet = LocalAccount.from_keystore(
            path=wallet_filepath, password=INSECURE_DEVELOPMENT_PASSWORD
        )
...
def test_create_wallet_from_keystore_file(mock_keystore):
    ...
    signer = LocalAccount.from_keystore(
        mock_keystore_filepath, password=INSECURE_DEVELOPMENT_PASSWORD
    )
