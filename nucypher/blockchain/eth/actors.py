...
class Operator(BaseActor):
    ...
    def _resolve_validators(
        ...
        for staking_provider_address, transcript_bytes in ritual.transcripts:
            if self.checksum_address == staking_provider_address:
                ...
                # Remote
                try:
                    remote_operator = self.peers[staking_provider_address]
                except KeyError:
    ...
    def perform_round_1(
        ...
        if pending_tx:
            self.log.debug(
                f"Node {self.wallet.address} has pending tx {pending_tx} for posting transcript for ritual {ritual_id}; skipping execution"
            )
            return None
