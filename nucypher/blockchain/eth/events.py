...
class EventRecord:
    def __init__(self, event: dict):
        ...
        try:
            ...
            self.timestamp = blockchain.client.w3.eth.get_block(self.block_number)[
                "timestamp"
            ]
...
class ContractEvents:
    ...
    def __get_web3_event_by_name(self, event_name: str):
        if event_name not in self.names:
            raise TypeError(
                f"Event '{event_name}' doesn't exist in this contract. Valid events are {self.names}"
            )
...
class ContractEventsThrottler:
    ...
    DEFAULT_MAX_BLOCKS_PER_CALL = int(
        os.environ.get(NUCYPHER_EVENTS_THROTTLE_MAX_BLOCKS, 1000)
    )
    ...
    def __init__(
        ...
        **argument_filters,
        ...
        if self.to_block < self.from_block:
            raise ValueError(
                f"Invalid events block range: to_block ({self.to_block}) must be ≥ "
    ...
    def __iter__(self):
        ...
        current_to_block = min(
            self.from_block + self.max_blocks_per_call, self.to_block
        )
