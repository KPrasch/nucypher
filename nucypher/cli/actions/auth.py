...
def get_password_from_prompt(
    prompt: str = GENERIC_PASSWORD_PROMPT, envvar: str = None, confirm: bool = False
) -> str:
...
def get_wallet_password(envvar: str = None, confirm: bool = False) -> str:
    ...
    client_password = get_password_from_prompt(
        prompt=COLLECT_ETH_PASSWORD, envvar=envvar, confirm=confirm
    )
    return client_password


def unlock_wallet(config: CharacterConfiguration) -> None:
    eth_password_is_needed = (not config.dev_mode,)
    __password = None
...
def get_nucypher_password(
    emitter, confirm: bool = False, envvar=NUCYPHER_ENVVAR_KEYSTORE_PASSWORD
) -> str:
    ...
    keystore_password = get_password_from_prompt(
        prompt=prompt, confirm=confirm, envvar=envvar
    )
...
def unlock_nucypher_keystore(
    ...
    emitter.message(
        DECRYPTING_CHARACTER_KEYSTORE.format(
            name=character_configuration.NAME.capitalize()
        ),
        color="yellow",
    )
    ...
    # unlock
    character_configuration.keystore.unlock(
        password=password
...
def recover_keystore(emitter) -> None:
    emitter.message(
        "This procedure will recover your nucypher keystore from mnemonic seed words. "
        "You will need to provide the entire mnemonic (space seperated) in the correct "
        "order and choose a new password.",
        color="cyan",
    )
    click.confirm("Do you want to continue", abort=True)

    __words = click.prompt("Enter mnemonic words", hide_input=True, type=str)

    word_count = len(__words.split())
    if word_count != mnemonic._WORD_COUNT:
        emitter.message(
            f"Invalid mnemonic - Number of words must be {str(mnemonic._WORD_COUNT)}, but only got {word_count}"
        )
    __password = get_nucypher_password(emitter=emitter, confirm=True)
    keystore = Keystore.restore(words=__words, password=__password)
    emitter.message(
        f"Recovered nucypher keystore {keystore.id} to \n {keystore.keystore_filepath}",
        color="green",
    )

    if not click.confirm(
        "Do you want to generate an ethereum wallet from these words?"
    ):
        return

    filepath = click.prompt(
        "Enter filepath to save wallet",
        type=Path,
        default=Keystore._DEFAULT_DIR / "operator.json",
    )
    account, fpath = LocalAccount.from_mnemonic(
        mnemonic=__words,
        password=get_wallet_password(
            envvar=NUCYPHER_ENVVAR_KEYSTORE_PASSWORD, confirm=True
        ),
        filepath=filepath,
    )
    emitter.message(
        f"Generated ethereum wallet {account.address} to \n {fpath}", color="green"
    )
